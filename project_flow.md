...
graph TD
    ...
    subgraph "UI Layer (PyQt5)"
        ...
        Tabs -->|Tab 3| DataTab[DataManagementTab]
        Tabs -->|Tab 4| EarlyTab[EarlyArrivalTab]
        Tabs -->|Tab 5| AttRepTab[AttendanceReportTab]
        Tabs -->|Tab 6| CompTab[ComparisonReportTab]
    ...
    class Entry,MW main;
    class RegTab,AbsTab,DataTab,EarlyTab,AttRepTab,CompTab tab;
...
4.  **Data Management**: All tabs interact with `DatabaseManager` to read/write from `data/database.json`.
5.  **Reporting**: `AttendanceReportTab` and `ComparisonReportTab` aggregate data and use `ExcelHandler` (or internal logic) to export reports.
